...
[`jwt.Keyfunc`](https://pkg.go.dev/github.com/golang-jwt/jwt/v4#Keyfunc). This function is for parsing and validating
JWTs.
...
a JWKs endpoint is using HTTPS to ensure the keys are from the correct trusted source.
...
The [`JWKs.Keyfunc`](https://pkg.go.dev/github.com/golang-jwt/jwt/v4/keyfunc#JWKs.Keyfunc) method will automatically
select the key with the matching `kid` (if present) and return its public key as the correct Go type to its caller.

## Test coverage

Test coverage is currently at `90.8%`.
...
* A background refresh of the JWKs keys can be performed. This is possible by passing
  [`keyfunc.Options`](https://pkg.go.dev/github.com/golang-jwt/jwt/v4/keyfunc#Options) via a variadic argument to the
  [`keyfunc.Get`](https://pkg.go.dev/github.com/golang-jwt/jwt/v4/keyfunc#Get) function.
